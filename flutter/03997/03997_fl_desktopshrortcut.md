...
이는 윈도우즈의 바로가기만 써도 충분히 구현이 가능하지만, 바탕화면을 정리하면서 바로가기 파일들이 자주 사라지기 때문에 만들게 되었다.
- 프로그램에 입력창같은 기능은 귀찮으니까, 제외를 하고, 같이 딸려있는 `json` 파일을 수정하는 형태로 구성한다.
...
&nbsp; <br>
### window manager
`pub.dev`에서 windows의 프로그램 창을 관리할 수 있는 첫번째 패키지를 선택했다. <br>
좋은건지 나쁜건지는 모르겠지만, 내가 사용할 기능정도는 있는 것같다.
&nbsp; <br>
- 초기화 : main()
```dart
...
&nbsp; <br>
- 초기화와 effect 설정 정도면 간단하게 끝이다. <br>
```dart
...
&nbsp; <br>
### system_tray
항상 떠 있는 창이다 보니 가끔 걸리적거릴 때가 있다. <br>
사용하지 않거나 화면을 많이 가리게 되는 경우는 system tray에 내려 둘 수 있도록 아래 패키지를 사용한다. <br>
