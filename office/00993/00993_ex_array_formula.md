...
하지만, 엑셀의 if 함수는 한가지 값만 반환을 하고, 여기서는 1을 반환한다. 따라서, sum(1)이 되니, 안타깝게도 답은 1이 된다. <br>
...
- 특정 글자가 포함된 조건 사용(와일드 카드 문자)
- model에 "Merc" 문자열이 포함되고, carb가 "4" 인 model 의 gear 값 합계
```vb
...
- 특정 글자가 포함된 조건 사용(와일드 카드 문자) + or 옵션
- model에 "Merc" 또는 "Mazda" 문자열이 포함되고, carb가 "4" 인 model 의 gear 값 합계
```vb
' 아래 두개 수식은 동일한 결과임
