...
## What is it??
`MacroToolkit`은 회사생활을 하면서 자주 사용했던 매크로들을 모아둔 `USERFORM`이다.<br>
`MacroToolkit`은 모든 상황에 대한 예외처리가 되어있지는 않기 때문에, 정해진 조건 내에서 사용하지 않으면, 특정 상황에서는 에러가 날 수 있다.
<br>
알겠지만 엑셀 VBA 사용 이후에는 Ctrl + z (UNDO) 기능이 적용되지 않는다.    
<br>
<br>
